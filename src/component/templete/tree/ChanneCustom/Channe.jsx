...
import { getdataallkey, getdataallkeyparent, getGameChanelist } from '../../../Utilities/CommonFuncs'
import './ChanneCustom.css'
...
class Channe extends React.Component {
    ...
    ajaxshow = (str) => {
        ...
        let tempchanne = [
            {"id":"50",name:"测试包",
            "children":[{"id":"7",name:"测试包一"}]},
            {"id":"500",name:"官方",
            "children":[{"id":"1",name:"官方1"},
            {"id":"2",name:"官方2"},
            {"id":"3",name:"官方3"},
            {"id":"4",name:"官方4"}
        ]}
        ]//getGameChanelist(str, tempdata)
        ...
        if (info.checked === false) {
            ...
            for (let j = 0; j < this.state.CheckedKeys.length; j += 1) {
                ...
                if (oldkey.indexOf(key) > -1) { //取消父节点时也会把其他的子节点同时取消
                    ...
                    // let tempval= newCheckedKeys.filter(i=>i!==oldkey)//新的过滤删除
                    // newCheckedKeys=tempval
                }
            }
        } else {
            //添加选中节点
        ...
        for (let i = (newCheckedKeys.length - 1); i >= 0; i--) {
            ...
            if (key.indexOf("-") > -1) {
                ...
                if (total > selected) {
                    ...
                    let tempval= newCheckedKeys.filter(i=>i!==key)//新的过滤删除
                    newCheckedKeys=tempval
                }
            }
        }
                            ...
                            if (datas[m].id === foundkey) {
                                ...
                                for (let j = presentcheckedList.length - 1; j >= 0; j--) {
                                    ...
                                    if (presentcheckedList[j] === foundkey || key === foundkey) {
                                        ...
                                        let presentcheckedListtemp= presentcheckedList.filter(i=>i!=presentcheckedList[j])
                                        presentcheckedList=presentcheckedListtemp
                                    }
                                }
        ...
        if (data !== "-1") {
            ...
            if (data.indexOf("-") === -1) {
                ...
                for (let i = 0; i < temp.length; i++) {
                    //  console.log("删除的值22",data,temp,temp[i].indexOf(data.toString()),)
                   if(temp[i].split("-")[0]!==data.toString()){
                    temp4.push(temp[i]) 
                   }
                }
                    temp = temp4;
            } else {
                // temp.remove(data)原始的删除
                console.log("走的这里",data)
                let tempval= temp.filter(i=>i!==data)//新的过滤删除
                temp=tempval
            }
