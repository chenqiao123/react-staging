...
//计划列表通用导出表格的数据转化
export const PlanProductionTable=(header,data)=>{
    ...
    //获得表头
    for(let i in header){
        if(header[i].key!="option"){
        obj.header.push(header[i].title)
        haskey.push(header[i].key)
    }
    }
    //获得数据
    for(let i=0;i<data.length;i++){
        for(let k in data[i]){
            if($.inArray(k,haskey)>-1){
                if(k==="insertTime"){
                    obj.data.push( MonthToday(data[i][k]))
                }else if(k==="status"){
                    let showvalue=""
                    switch(data[i][k]){

                        case 0:showvalue="未激活";break;
